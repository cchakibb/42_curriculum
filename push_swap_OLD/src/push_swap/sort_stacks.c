...
{
	while ((*a)->nbr != get_min(*a)->nbr) //As long as the smallest number is not at the top
	{
		if (get_min(*a)->above_median) //Rotate or reverse rotate according to the position of the node on the median
			ra(a, false);
		else
			rra(a, false);
	}
